...
const getMissionIcon = (id: number, isSelected: boolean) => {
  const iconClass = cn(
    "h-8 w-8",
...
export const MissionCard = ({
  ...
  const isSelected = mission.id === id;
  ...
  return (
    <div
      ...
      onClick={() => setMission({ id, title, description })}
    >
      <div className="mb-4">{getMissionIcon(id, isSelected)}</div>
      <p
        className={cn(
